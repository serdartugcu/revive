...
/**
 * Builds header model for network edit page tabs
 *
 * @param mixed $idOrNetwork - either network id or network array
 * @return OA_Admin_UI_Model_PageHeaderModel
 */
function buildNetworkHeaderModel($idOrNetwork)
{
    if (is_array($idOrNetwork)) {
        $aNetwork = $idOrNetwork;
    }
    else if (!empty($idOrNetwork)) {
        $aNetwork = phpAds_getNetworkDetails($idOrNetwork);
    }
    else {
        $aNetwork = array();
    }
    ...
    $builder = new OA_Admin_UI_Model_InventoryPageHeaderModelBuilder();
    $pageType = empty($aNetwork['networkid']) ? 'edit-new' : 'edit';

    $oHeaderModel = $builder->buildEntityHeader(array(
    array("name" => $aNetwork['name'])),
    "network", $pageType);
    return $oHeaderModel;
}
...
function MAX_displayNetworkBreadcrumbs($networkid)
{
	if ($networkid) {
		$pageType = "edit";
	}
	...
	$builder = new OA_Admin_UI_Model_InventoryPageHeaderModelBuilder();
	$oHeaderModel = $builder->buildEntityHeader(array(
			array("name" => $networkName)), "network", $pageType);
...
function addBannerPageTools($advertiserId, $campaignId, $bannerId, $aOtherCampaigns, $aOtherBanners, $aEntities)
{
    ...
    //delete
    if (!OA_Permission::isAccount(OA_ACCOUNT_ADVERTISER)) {
    	$confirmBanner = phpAds_DelConfirm('Confirm Banner');
    	addPageLinkTool($GLOBALS["strDelete"], MAX::constructUrl(MAX_URL_ADMIN, "banner-edit.php?clientid=$advertiserId&campaignid=$campaignId&bannerid=$bannerId&returnurl=campaign-banners.php"), "iconDelete", null, $confirmBanner);
    }
