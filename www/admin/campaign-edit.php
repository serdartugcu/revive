...
if ($campaignid != "") {
    ...
    $campaign['clicksRemaining'] = '';
    $campaign['conversionsRemaining'] = '';
    $campaign['totalBudget'] = $data['total_budget'];
    $campaign['dailyBudget'] = $data['daily_budget'];
    $campaign['agentPercentage'] = $data['agent_percentage'];
    ...
    $campaign['agent_percentage'] = null;
}
...
function buildPricingFormSection(&$form, $campaign, $newCampaign, $remnantEcpmEnabled, $contractEcpmEnabled)
{
    ...
    //TODO CPV pricing model - add the related code
    $aRevenueTypes[MAX_FINANCE_CPV] = $GLOBALS ['strFinanceCPV'];
    ...
    $totalBudget['field'] = $form->createElement ( 'text', 'totalBudget', null );
   	$form->addGroup ($totalBudget, 'g_totalBudget', 'Total Budget') ;
   	$dailyBudget['field'] = $form->createElement( 'text', 'dailyBudget', null);
   	$form->addGroup($dailyBudget, 'g_dailyBudget', 'Daily Budget');
   	$agentPercentage['field'] = $form->createElement('text', 'agentPercentage', null);
   	$form->addGroup($agentPercentage, 'g_agentPercentage', 'Agent Percentage');
...
function processCampaignForm($form, &$oComponent = null)
{
    ...
    if (empty($errors)) {
        ...
        else if ($aFields['revenue_type'] == MAX_FINANCE_CPV) {		//TODO add CPV code
        	$aFields['conversions'] = - 1;
        }
        ...
        $doCampaigns->total_budget = $aFields['totalBudget'];
        $doCampaigns->daily_budget = $aFields['dailyBudget'];
        $doCampaigns->agent_percentage = $aFields['agentPercentage'];
...
function displayPage($campaign, $campaignForm, $statusForm, $campaignErrors = null)
{
    ...
    $oTpl->assign ( 'MODEL_CPV', MAX_FINANCE_CPV );
