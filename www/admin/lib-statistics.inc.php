...
/*-------------------------------------------------------*/
/* Build the network name from ID and name             */
/*-------------------------------------------------------*/

function phpAds_buildNetworkName($affiliateid, $name)
{
	return ("<span dir='".$GLOBALS['phpAds_TextDirection']."'>[id$networkid]</span> " . htmlspecialchars($name));
}
