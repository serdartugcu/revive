...
function buildBannerForm($type, $aBanner, &$oComponent=null, $formDisabled=false)
{
    ...
    $form->addElement('header', 'header_confirm', 'Confirm Banner');
    $confirmBannerCB = $form->createElement('advcheckbox', 'confirmBanner', 'Confirm this banner', null, array('group' => 1), array(0, 1));
    $confirmBannerCB->setChecked(TRUE);
    $form->addElement($confirmBannerCB);
    ...
    //Serdar: Select advertiser
    $form->addElement('header', 'header-additional', 'Advertiser');
    $dalAffiliates = OA_Dal::factoryDAL('affiliates');
    $aWebsitesZones = $dalAffiliates->getWebsites();

    $combobox = $form->addElement('Select', 'combobox', 'website', $aWebsitesZones);
