...
class OA_UpgradePostscript_2_7_11_dev
{
    ...
    function execute($aParams)
    {
        ...
        */
        ...
        $cpvCount = count($aCPVCampaigns);
        
        $count = $cpmCount +  $cpcCount + $cpaCount + $cpvCount;
        ...
        if ($cpvCount > 0) {
        	$query = sprintf($this->queryUpdateTemplate, MAX_FINANCE_CPV, implode(',', $aCPVCampaigns));
            $result = $this->oDbh->exec($query);
            if (PEAR::isError($result))
            {
                $this->logError($result->getUserInfo());
            ...
            {
                $this->logOnly("Successfully updated 'revenue_type' of $result campaign(s) to CPV");
            }
        }
        return true;
    }
